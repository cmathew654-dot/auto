...
function normalizeAccountType(raw) {
    ...
    if (['taxable brokerage', 'brokerage', 'individual', 'joint'].includes(v))
        return 'Taxable Brokerage';
    if (['traditional ira', 'ira', 'rollover ira'].includes(v))
        return 'Traditional IRA';
    if (['roth ira'].includes(v))
        return 'Roth IRA';
    if (['401k', '403b', 'retirement plan'].includes(v))
...
function normalizeDuplicateText(raw) {
...
function detectHeaderRow(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        for (const aliases of Object.values(HEADER_ALIASES)) {
            if (normalized.some((cell) => aliases.includes(cell)))
                score++;
        }
        if (score > bestScore) {
            bestScore = score;
...
function toIssue(message, source, code = 'VALIDATION_WARNING', blocking = false, field) {
    return {
        code,
...
function makeHoldingId(accountId, index) {
...
function repairUnquotedThousandsSeparators(row, expectedLength, headerMap) {
    var _a, _b, _c, _d;
    if (row.length <= expectedLength)
        return row;
    const repaired = [...row];
    const numericFields = ['units', 'price', 'marketValue', 'costBasis'];
    let guard = 0;
    while (repaired.length > expectedLength && guard < 20) {
        ...
        for (const field of numericFields) {
            ...
            if (idx === undefined || idx >= repaired.length - 1)
                continue;
            const left = (_b = (_a = repaired[idx]) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : '';
            const right = (_d = (_c = repaired[idx + 1]) === null || _c === void 0 ? void 0 : _c.trim()) !== null && _d !== void 0 ? _d : '';
...
export function parseHoldingsCsvToIngestionFile(csvText, opts) {
    var _a, _b, _c, _d, _e, _f;
    // Keep only non-empty physical lines; header detection is run on these rows.
    const lines = csvText.split(/\r?\n/).filter((line) => line.trim().length > 0);
    const rows = lines.map(parseCsvLine);
    const headerIndex = detectHeaderRow(rows);
    const header = (_a = rows[headerIndex]) !== null && _a !== void 0 ? _a : [];
    const headerMap = buildHeaderMap(header);
    const startedAt = new Date().toISOString();
    const fileIssues = [];
    const requiredSignal = ['ticker', 'cusip', 'description', 'units', 'accountNumber'];
    const foundSignals = requiredSignal.filter((k) => headerMap[k] !== undefined).length;
    if (foundSignals < 2) {
        fileIssues.push(toIssue('Header detection confidence is low; review mapped columns before entry.', 'validation'));
    }
    ...
    for (let i = headerIndex + 1; i < rows.length; i++) {
        ...
        if (isBlankRow(row))
            continue;
        const accountNumber = (_b = getCell(row, headerMap.accountNumber)) !== null && _b !== void 0 ? _b : 'UNASSIGNED';
        ...
        const firstCell = normalizeHeader((_c = row[0]) !== null && _c !== void 0 ? _c : '');
        if (looksLikeTotalRow(row) && (firstCell === 'total' || normalizeHeader(accountNumber) === 'total')) {
            fileIssues.push(toIssue(`Skipped likely workbook total row at CSV line ${i + 1}.`, 'ingestion', 'VALIDATION_WARNING', false));
            continue;
        }
        const rawAccountType = getCell(row, headerMap.accountType);
        const normalizedAccountType = normalizeAccountType(rawAccountType);
        const accountType = normalizedAccountType !== null && normalizedAccountType !== void 0 ? normalizedAccountType : 'Unknown';
        ...
        const accountKey = normalizedAccountType
            ...
            : `${accountNumber}::UNMAPPED:${rawAccountType !== null && rawAccountType !== void 0 ? rawAccountType : 'MISSING'}`;
        if (!accountBuckets.has(accountKey)) {
            ...
            accountBuckets.set(accountKey, {
                ...
                issues: [],
                skippedTotalRows: 0,
                seenHoldingKeys: new Set(),
            });
        }
        const bucket = accountBuckets.get(accountKey);
        if (rawAccountType && !normalizedAccountType) {
            bucket.issues.push(toIssue(`Unmapped account type "${rawAccountType}" at CSV line ${i + 1}.`, 'validation', 'UNMAPPED_ACCOUNT_TYPE', false, 'accountType'));
        }
        if (looksLikeTotalRow(row)) {
            bucket.skippedTotalRows += 1;
            bucket.issues.push(toIssue(`Skipped likely subtotal/total row at CSV line ${i + 1}.`, 'ingestion', 'VALIDATION_WARNING', false));
            continue;
        }
        ...
        if (!ticker && !cusip && !description) {
            bucket.issues.push(toIssue(`Skipped row at CSV line ${i + 1}: no ticker, CUSIP, or description.`, 'validation', 'MISSING_REQUIRED_FIELD', false));
            continue;
        }
        const rowIssues = [];
        const units = normalizeNumber(unitsRaw !== null && unitsRaw !== void 0 ? unitsRaw : undefined);
        const price = normalizeNumber(priceRaw !== null && priceRaw !== void 0 ? priceRaw : undefined);
        const costBasis = normalizeNumber(costBasisRaw !== null && costBasisRaw !== void 0 ? costBasisRaw : undefined);
        const marketValue = normalizeNumber(marketValueRaw !== null && marketValueRaw !== void 0 ? marketValueRaw : undefined);
        if (unitsRaw && units === null) {
            rowIssues.push(toIssue(`Could not parse units value "${unitsRaw}".`, 'validation', 'INVALID_FORMAT', false, 'units'));
        }
        if (priceRaw && price === null) {
            rowIssues.push(toIssue(`Could not parse price value "${priceRaw}".`, 'validation', 'INVALID_FORMAT', false, 'price'));
        }
        if (costBasisRaw && costBasis === null) {
            rowIssues.push(toIssue(`Could not parse cost basis value "${costBasisRaw}".`, 'validation', 'INVALID_FORMAT', false, 'costBasis'));
        }
        if (marketValueRaw && marketValue === null) {
            rowIssues.push(toIssue(`Could not parse market value "${marketValueRaw}".`, 'validation', 'INVALID_FORMAT', false, 'marketValue'));
        }
        const normalizedTicker = normalizeDuplicateText(ticker !== null && ticker !== void 0 ? ticker : '');
        const rawAssetClass = (_e = (_d = row[header.length - 1]) === null || _d === void 0 ? void 0 : _d.trim()) !== null && _e !== void 0 ? _e : '';
        const isCashRow = normalizedTicker === 'cash' || normalizeDuplicateText(rawAssetClass) === 'cash';
        if (isCashRow) {
            rowIssues.push(toIssue('Cash row requires manual handling and is excluded from eMoney public-security auto-entry by default.', 'validation', 'CASH_SPECIAL_HANDLING', false, 'ticker'));
        }
        if (!isCashRow && price === 0 && marketValue != null && marketValue !== 0) {
            rowIssues.push(toIssue('Zero price with nonzero market value requires manual review before eMoney auto-entry.', 'validation', 'ZERO_PRICE_NONZERO_VALUE_EXCEPTION', false, 'price'));
        }
        if (acquisitionDate && !isValidIsoDate(acquisitionDate)) {
            rowIssues.push(toIssue(`Could not parse acquisition date "${acquisitionDate}". Expected YYYY-MM-DD.`, 'validation', 'INVALID_FORMAT', false, 'acquisitionDate'));
        }
        ...
        if (cusip)
            duplicateKeyCandidates.push(`cusip:${normalizeDuplicateText(cusip)}`);
        if (ticker && description) {
            duplicateKeyCandidates.push(`ticker_desc:${normalizeDuplicateText(ticker)}|${normalizeDuplicateText(description)}`);
        }
        if (ticker && units != null)
            duplicateKeyCandidates.push(`ticker_units:${normalizeDuplicateText(ticker)}|${units}`);
        ...
        if (duplicateKeyCandidates.some((key) => bucket.seenHoldingKeys.has(key))) {
            rowIssues.push(toIssue('Likely duplicate holding detected within the same account.', 'validation', 'DUPLICATE_HOLDING', false));
        }
    ...
    const accounts = Array.from(accountBuckets.entries()).map(([key, bucket]) => {
        ...
        const reviewStatus = allHoldingIssues > 0 || bucket.issues.length > 0 ? 'in_review' : 'unreviewed';
        const entryStatus = bucket.holdings.length === 0 ? 'needs_human' : 'not_started';
        return {
            ...
            totals: {
                ...
                totalMarketValue: hasAnyMarketValue
                    ? bucket.holdings.reduce((sum, h) => { var _a; return sum + ((_a = h.marketValue) !== null && _a !== void 0 ? _a : 0); }, 0)
                    : null,
                totalCostBasis: hasAnyCostBasis
                    ? bucket.holdings.reduce((sum, h) => { var _a; return sum + ((_a = h.costBasis) !== null && _a !== void 0 ? _a : 0); }, 0)
                    : null,
            },
        };
    });
    const warningCount = fileIssues.filter((x) => x.severity === 'warning').length +
        accounts.reduce((sum, a) => sum + a.issues.filter((x) => x.severity === 'warning').length + a.holdings.reduce((s, h) => s + h.issues.filter((x) => x.severity === 'warning').length, 0), 0);
    const errorCount = fileIssues.filter((x) => x.severity === 'error').length +
        accounts.reduce((sum, a) => sum + a.issues.filter((x) => x.severity === 'error').length + a.holdings.reduce((s, h) => s + h.issues.filter((x) => x.severity === 'error').length, 0), 0);
    const endedAt = new Date().toISOString();
    // Top-level reviewStatus should reflect issues at every level, not only file-level issues.
    const hasAnyIssues = fileIssues.length > 0 ||
        accounts.some((a) => a.issues.length > 0 || a.holdings.some((h) => h.issues.length > 0));
    return {
        schemaVersion: '1.0.0',
        fileId: (_f = opts === null || opts === void 0 ? void 0 : opts.fileId) !== null && _f !== void 0 ? _f : `file-${startedAt}`,
        createdAt: startedAt,
        ...
        runLog: {
            ...
            ...((opts === null || opts === void 0 ? void 0 : opts.machine) ? { machine: opts.machine } : {}),
            summary: {
                accountsTotal: accounts.length,
                holdingsTotal: accounts.reduce((sum, a) => sum + a.holdings.length, 0),
                enteredCount: 0,
                needsHumanCount: accounts.reduce((sum, a) => sum + a.holdings.filter((h) => h.entryStatus === 'needs_human').length, 0),
                failedCount: 0,
                warningCount,
...
`;
export const sampleParsedShape = {
    schemaVersion: '1.0.0',
    workflow: {
        localOnly: true,
        backendUsed: false,
