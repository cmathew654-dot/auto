 ...
 */
import { parseHoldingsCsvToIngestionFile } from './holdings-csv-parser.js';
import { installRegulatedLedgerStyles } from './ledger-styles.js';
import { renderReviewExportSurface } from './review-export-surface.js';
...
export function runLocalMvp(container, csvText = SAMPLE_CSV_INPUT, opts) {
    var _a;
...
async function stageLoadStatus(status, sourceName) {
...
function summarizeSessionAccount(ingestion) {
    if (ingestion.accounts.length === 0)
        return 'No account detected';
    if (ingestion.accounts.length === 1)
        return ingestion.accounts[0].accountNumber;
    return `${ingestion.accounts[0].accountNumber} + ${ingestion.accounts.length - 1} more`;
}
function renderLedgerSkeleton(container, sourceName) {
    container.innerHTML = [
...
export function renderLocalMvpShell(root) {
    ...
    buildPill.textContent = (buildInfo === null || buildInfo === void 0 ? void 0 : buildInfo.sha)
    ...
    const setWorkflowStep = (active) => {
        const rank = { load: 0, review: 1, packet: 2, fill: 3 };
    ...
    const markSessionActive = (ingestion) => {
