import { buildBatchPastePayload, buildEmoneyFillBookmarklet, buildEmoneyFillPacket, serializeEmoneyFillPacket, } from './paste-conductor.js';
export const MANUAL_REVIEW_REQUIRED_CODES = new Set([
    'DUPLICATE_HOLDING',
    'INVALID_FORMAT',
...
function toFriendlyIssueLabel(code) {
    switch (code) {
...
export function getHoldingEligibility(holding, opts) {
    var _a, _b;
    const reasons = [];
    const blockedIssueCodes = [];
    const hasLookupKey = Boolean(((_a = holding.ticker) !== null && _a !== void 0 ? _a : '').trim() || ((_b = holding.cusip) !== null && _b !== void 0 ? _b : '').trim());
    if (!hasLookupKey) {
    ...
    if (requiresManualOverride && !(opts === null || opts === void 0 ? void 0 : opts.allowManualOverride)) {
...
export function isHoldingEligible(holding, opts) {
    return getHoldingEligibility(holding, opts).eligible;
}
export function toAssistantHolding(holding) {
    var _a, _b, _c, _d, _e, _f;
    return {
        ticker: (_a = holding.ticker) !== null && _a !== void 0 ? _a : null,
        cusip: (_b = holding.cusip) !== null && _b !== void 0 ? _b : null,
        description: (_c = holding.description) !== null && _c !== void 0 ? _c : null,
        units: (_d = holding.units) !== null && _d !== void 0 ? _d : null,
        costBasis: (_e = holding.costBasis) !== null && _e !== void 0 ? _e : null,
        marketValue: (_f = holding.marketValue) !== null && _f !== void 0 ? _f : null,
    };
}
export function buildAccountPreflightSummary(account, opts) {
    const blockedReasonsByCode = {};
    let eligibleCount = 0;
    let blockedCount = 0;
    for (const holding of account.holdings) {
        ...
        if (eligibility.eligible) {
            ...
            eligibility.blockedIssueCodes.forEach((code) => {
                var _a;
                blockedReasonsByCode[code] = ((_a = blockedReasonsByCode[code]) !== null && _a !== void 0 ? _a : 0) + 1;
            });
        }
    }
...
export function toAssistantPayloadForAccount(account, opts) {
...
export function buildEmoneyDevtoolsSnippet(payload) {
    const rows = payload.holdings
        .filter((holding) => { var _a; return ((_a = holding.ticker) !== null && _a !== void 0 ? _a : '').trim(); })
        .map((holding) => {
        var _a;
        return ({
            ticker: String((_a = holding.ticker) !== null && _a !== void 0 ? _a : '').trim(),
...
export function getAccountRowDisplay(account, opts) {
    return account.holdings.map((holding) => {
        ...
        return {
            ...
            marketValue: (_a = holding.marketValue) !== null && _a !== void 0 ? _a : null,
        };
    });
}
function formatIssues(issues) {
    if (!issues.length)
        return 'none';
    return issues
        .map((i) => `${i.severity.toUpperCase()}:${i.code}${i.field ? ` (${i.field})` : ''} - ${i.message}`)
        .join(' | ');
}
function makeButton(label, className = 'ledger-button') {
...
function formatFieldNames(fields) {
    ...
    return fields.map((field) => { var _a; return (_a = labels[field]) !== null && _a !== void 0 ? _a : field; }).join(', ');
}
...
function appendFieldList(parent, title, fields, className) {
...
function appendReasonChips(cell, codes, fallback) {
    if (!codes.length) {
...
function buildBatchTransferReport(batch) {
    const lines = [
...
function buildFillPacketReport(packet) {
    const lines = [
        'eMoney Fill Packet',
...
function downloadFillPacketReport(packet) {
...
export function renderReviewExportSurface(root, ingestion, opts) {
    root.innerHTML = '';
    ...
    const showOutput = (message, opts) => {
        var _a, _b;
        output.textContent = message;
        outputDetails.open = (_a = opts === null || opts === void 0 ? void 0 : opts.open) !== null && _a !== void 0 ? _a : false;
        outputSummary.textContent = (_b = opts === null || opts === void 0 ? void 0 : opts.summary) !== null && _b !== void 0 ? _b : 'Session report available';
    };
    ingestion.accounts.forEach((account) => {
        ...
        let activeFillPacket = null;
        let activeManualBatch = null;
        let packetCopyStatus = 'idle';
        let packetCopiedAt = null;
        ...
        const renderRows = () => {
            ...
            rowDisplay.forEach(({ holding, eligible, blockedWhy, blockedIssueCodes }, index) => {
                ...
                tr.setAttribute('aria-label', `${eligible ? 'Eligible' : 'Blocked'} holding ${(_b = (_a = holding.ticker) !== null && _a !== void 0 ? _a : holding.cusip) !== null && _b !== void 0 ? _b : 'without lookup key'}`);
                if (!eligible) {
                    tr.title = blockedWhy;
                ...
                appendCell(tr, (_c = holding.ticker) !== null && _c !== void 0 ? _c : '');
                appendCell(tr, (_d = holding.cusip) !== null && _d !== void 0 ? _d : '');
                appendCell(tr, holding.units == null ? '' : String(holding.units));
                appendCell(tr, holding.costBasis == null ? '' : formatMoney(holding.costBasis));
                appendCell(tr, holding.marketValue == null ? '' : formatMoney(holding.marketValue));
        ...
        const renderTransferPacket = () => {
            ...
            if (!activeFillPacket || !activeManualBatch)
                return;
            ...
            copyBtn.onclick = async () => {
                var _a, _b;
                if (!activeFillPacket || activeFillPacket.rowCount === 0)
                    return;
                ...
                (_a = opts === null || opts === void 0 ? void 0 : opts.onPacketPrepared) === null || _a === void 0 ? void 0 : _a.call(opts, {
                    accountNumber: activeFillPacket.accountNumber,
                    rowCount: activeFillPacket.rowCount,
                    copied,
                });
                (_b = opts === null || opts === void 0 ? void 0 : opts.onExport) === null || _b === void 0 ? void 0 : _b.call(opts, payload);
                showOutput(copied
            ...
            copyBookmarkBtn.onclick = async () => {
                ...
                if (copied) {
                    (_a = opts === null || opts === void 0 ? void 0 : opts.onPacketPrepared) === null || _a === void 0 ? void 0 : _a.call(opts, {
                        accountNumber: account.accountNumber,
                        rowCount: (_b = activeFillPacket === null || activeFillPacket === void 0 ? void 0 : activeFillPacket.rowCount) !== null && _b !== void 0 ? _b : 0,
                        copied: true,
                    });
                }
                showOutput(copied
                    ...
                    : `Clipboard copy was blocked. Manually copy this bookmark URL into a Chrome bookmark:\n${bookmarkletHref}`, { summary: copied ? 'Bookmark URL copied' : 'Bookmark URL copy blocked', open: !copied });
            };
            ...
            fallbackBtn.onclick = async () => {
                if (!activeManualBatch || activeManualBatch.rowCount === 0)
                    return;
        ...
        exportBtn.onclick = () => {
            ...
            (_a = opts === null || opts === void 0 ? void 0 : opts.onExport) === null || _a === void 0 ? void 0 : _a.call(opts, payload);
        };
